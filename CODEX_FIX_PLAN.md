...
File: backend/src/app/main.py
Problem: ensure_column() runs on every startup to patch schema — SQLite-only, won't work on PostgreSQL.
Fix:
...
Fix:
  - Create a test database fixture (use SQLite in-memory for tests: sqlite:///):
    ```python
    @pytest.fixture
    def db():
...
```
DATABASE_URL=postgresql://user:password@localhost:5432/syra_lms
