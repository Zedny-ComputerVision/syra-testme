...
describe('AdminAttemptVideos supervision mode', () => {
  ...
  it('anchors warning timestamps to the selected recording window instead of the attempt start', async () => {
    getAttemptMock.mockResolvedValue({
      data: {
        id: 'attempt-7',
        status: 'SUBMITTED',
      ...
      data: [
        {
          ...
          severity: 'HIGH',
          detail: 'Face not detected for 6.0s',
          occurred_at: '2026-03-07T10:04:24Z',
        },
        {
          id: 'event-outside',
          event_type: 'CAMERA_COVERED',
          severity: 'HIGH',
          detail: 'Camera covered',
          occurred_at: '2026-03-07T10:02:24Z',
        },
      ],
    })
    ...
    await waitFor(() => expect(screen.getAllByText('FACE_DISAPPEARED').length).toBeGreaterThan(0))
    await waitFor(() => expect(screen.getByText('0:16')).toBeTruthy())
    expect(screen.queryByText('Camera covered')).toBeNull()
  })
})
