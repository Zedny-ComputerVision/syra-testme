...
from ...modules.tests.models import Test  # noqa: F401
...
def _ensure_export_access(current: User, target_user: User) -> None:
...
def _file_metadata_from_path(value: str | None) -> dict | None:
    if not value:
        return None
...
def _collect_attempt_files(directory: Path, attempt_id) -> list[dict]:
...
def _collect_attempt_media(attempt: Attempt) -> dict:
    ...
    return {
        ...
        "identity_documents": [
            item
            for item in (
                _file_metadata_from_path(attempt.id_doc_path),
                _file_metadata_from_path(attempt.selfie_path),
            )
            if item
        ],
    }
...
async def export_user_data(
    ...
    _ensure_export_access(current, target_user)
    ...
    schedules = db.scalars(
        select(Schedule)
        .options(joinedload(Schedule.exam), joinedload(Schedule.test))
        ...
        "attempts": [
            {
                ...
                "proctoring_media": _collect_attempt_media(attempt),
            }
        ...
        "schedules": [
            {
                ...
                "test_title": (
                    schedule.exam.title
                    if schedule.exam
                    else getattr(schedule.test, "name", None)
                ),
